...
class Bot2:
    ...
    def analyze(self, symbol: str) -> Optional[Position]:
        ...
        rsi = ta.momentum.RSIIndicator(df30["close"], window=14).rsi()
        ema60_fast = ta.trend.EMAIndicator(df60["close"], window=20).ema_indicator()
        ema60_slow = ta.trend.EMAIndicator(df60["close"], window=50).ema_indicator()
        ...
        uptrend = ema60_fast.iloc[-1] > ema60_slow.iloc[-1]
        downtrend = ema60_fast.iloc[-1] < ema60_slow.iloc[-1]

        size = self.usdt_per_trade / price
        if price < lower and rsi.iloc[-1] < 30 and uptrend:
            sl = price - atr_val * 1.2
            tp = price + atr_val * 2.4
            return Position(symbol, "long", price, size, sl, tp)
        if price > upper and rsi.iloc[-1] > 70 and downtrend:
            sl = price + atr_val * 1.2
            tp = price - atr_val * 2.4
